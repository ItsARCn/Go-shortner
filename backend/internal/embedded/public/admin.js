document.addEventListener('DOMContentLoaded', async () => {
  ...
  const badgePerms = document.getElementById('badge-perms-count');
  ...
  async function loadOverview() {
    try {
      ...
      if (badgeReports) badgeReports.textContent = d.reports_count;

      // Also fetch pending perms count
      const permRes = await fetch('/api/admin/permanent-requests?status=pending');
      if (permRes.ok) {
        const pd = await permRes.json();
        if (badgePerms) badgePerms.textContent = pd.total;
      }
    } catch (err) {}
  }
  ...
  function renderUsers(users) {
          ...
          <td style="text-align:right;">
            <div class="table-actions">
              ${isSuperAdmin && u.id !== currentUser.id ? `
                <button class="action-btn change-role-btn" data-id="${u.id}" data-email="${escapeHTML(u.email)}" data-role="${u.role}">Role</button>
              ` : ''}
    ...
    // Attach actions
    document.querySelectorAll('.change-role-btn').forEach(b => {
      b.addEventListener('click', () => {
        openRoleModal(b.getAttribute('data-id'), b.getAttribute('data-email'), b.getAttribute('data-role'));
      });
    });
  ...
  // 5. PERMANENT LINK REQUESTS
  let permStatusFilter = 'pending';
  const permsTbody = document.getElementById('admin-perms-tbody');

  document.querySelectorAll('.perm-filter-status').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('.perm-filter-status').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      permStatusFilter = btn.getAttribute('data-status');
      loadPerms();
    });
  });
  ...
  async function loadPerms() {
    try {
      const res = await fetch(`/api/admin/permanent-requests?status=${permStatusFilter}`);
      if (!res.ok) {
        permsTbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#f43f5e; padding:2rem;">Failed to load permanent requests</td></tr>`;
        return;
      }
      const data = await res.json();
      renderPerms(data.requests);
    } catch (err) {
      permsTbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#f43f5e; padding:2rem;">Network error</td></tr>`;
    }
  }

  function renderPerms(requests) {
    if (!requests || requests.length === 0) {
      permsTbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:var(--text-dim); padding:2rem;">No ${permStatusFilter} permanent link requests.</td></tr>`;
      return;
    }

    permsTbody.innerHTML = requests.map(p => `
      <tr>
        <td>
          <a href="/${p.short_code}" target="_blank" class="link-code">/${p.short_code}</a>
        </td>
        <td><div class="dest-url" title="${escapeHTML(p.destination_url)}">${escapeHTML(p.destination_url)}</div></td>
        <td style="font-size:0.85rem; color:var(--text-muted);">${escapeHTML(p.user_email)}</td>
        <td style="font-size:0.85rem; max-width:200px;">${escapeHTML(p.reason)}</td>
        <td><span class="click-chip">${p.status}</span></td>
        <td style="font-size:0.8rem; color:var(--text-dim);">${new Date(p.created_at).toLocaleDateString()}</td>
        <td style="text-align:right;">
          <div class="table-actions">
            ${p.status === 'pending' ? `
              <button class="action-btn approve-perm-btn" data-id="${p.id}" style="color:#34d399;">Approve</button>
              <button class="action-btn reject-perm-btn" data-id="${p.id}" style="color:#fb7185;">Reject</button>
            ` : `
              <span style="font-size:0.8rem; color:var(--text-dim);">${p.status}</span>
            `}
          </div>
        </td>
      </tr>
    ...
    document.querySelectorAll('.approve-perm-btn').forEach(b => {
      b.addEventListener('click', async () => {
        const id = b.getAttribute('data-id');
        if (confirm('Approve permanent status? This link will never expire and auto-renew indefinitely.')) {
          await fetch(`/api/admin/permanent-requests/${id}/resolve`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    document.querySelectorAll('.reject-perm-btn').forEach(b => {
      b.addEventListener('click', async () => {
        const id = b.getAttribute('data-id');
        if (confirm('Reject permanent status request?')) {
          await fetch(`/api/admin/permanent-requests/${id}/resolve`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  function openRoleModal(id, email, currentRole) {
    ...
    document.getElementById('role-target-email').textContent = email;
  ...
  document.getElementById('close-role-modal').addEventListener('click', () => roleModal.classList.remove('show'));
  document.getElementById('cancel-role-btn').addEventListener('click', () => roleModal.classList.remove('show'));
